...
# В этом примере возможны несколько вариантов логики метода auto_start().
# Первый вариант — при передаче в метод одного параметра.
# Второй — при передаче двух параметров.
# В первом случае будет выведено значение переданного параметра, во втором — сумма параметров.
...
# Переопределение методов в полиморфизме выражается в наличии метода с одинаковым названием для родительского
# и дочернего классов.
...
# В примере классы Auto и Bus наследуют характеристики класса Transport.
# В этом классе реализуется метод show_info(), переопределенный классом-потомком.
...
# В этом примере методы show_info() вызываются с помощью производных классов одного общего базового класса.
# Но дочерние классы переопределяются через метод класса-родителя, методы обладают разной функциональностью.